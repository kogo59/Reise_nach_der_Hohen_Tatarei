BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Reise_nach_der_Hohen_Tatarei
TITLE_NAME = "Reise nach der Hohen Tatarei, Yârkand und Kâshghar und Rückreise über den Karakoram-Pass"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Reise_nach_der_Hohen_Tatarei_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Reise_nach_der_Hohen_Tatarei_*.jpg  cover.jpg logo.jpg
